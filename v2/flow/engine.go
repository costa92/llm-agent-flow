...
func (e *Engine) Run(ctx context.Context, inputs map[string]any) (map[string]any, error) {
	return e.runAny(ctx, inputs, nil)
}
...
func (e *Engine) RunStream(ctx context.Context, inputs map[string]any) (<-chan Event, error) {
	...
	go func() {
		defer close(ch)
		_, _ = e.runAny(ctx, inputs, ch)
	}()
	return ch, nil
}

// runAny is the shared core — a line-by-line port of the v0.1
// flow.Engine.run loop to the any carrier. ch may be nil for the sync
// path.
func (e *Engine) runAny(ctx context.Context, inputs map[string]any, ch chan<- Event) (map[string]any, error) {
	...
	// Flow.Inputs always pre-activate their target nodes — they receive
	// a value from the caller, so the node has work to do even if it has
	// no incoming edges (the common entry-node case).
		...
		for _, srcID := range layer {
			...
			for edgeIdx, edge := range e.flow.Edges {
				...
				if cond != nil {
					...
					// edge is a run-time error (the typed Branch
					// front-end, not CEL, handles non-string routing).
					sval, ok := value.(string)
					if !ok {
						err := fmt.Errorf("flow: run: edge[%d] (%s.%s → %s.%s) condition: source value is %T, not string — conditional (CEL) edges route on string values only",
							edgeIdx, edge.Source.Node, edge.Source.Port, edge.Target.Node, edge.Target.Port, value)
						emit(Event{Kind: FlowErr, Err: err})
						return nil, err
					}
					...
					if evalErr != nil {
						...
						emit(Event{Kind: FlowErr, Err: wrapped})
						return nil, wrapped
					}
					if !fire {
						continue
