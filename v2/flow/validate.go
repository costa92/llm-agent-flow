...
// resolve node types — that happens at Engine construction. The checks
// here only need the IR shape:
//
//   - non-empty node list
//   - unique node IDs
...
// findCycle walks the directed edge set with iterative DFS and returns
// one cycle (as the visited node-id sequence) if one exists. Returns
