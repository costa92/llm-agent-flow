...
// update, when set, rewrites the committed api/v2.0.snapshot.txt baseline
// instead of comparing against it. Use for deliberate v2.0-additive changes:
//
//	go test ./internal/apisnapshot/ -run TestAPISnapshot -update
var update = flag.Bool("update", false, "rewrite api/v2.0.snapshot.txt")
...
func snapshotPath(root string) string {
	return filepath.Join(root, "api", "v2.0.snapshot.txt")
}
...
func TestAPISnapshot(t *testing.T) {
	...
	t.Fatalf("exported API surface changed — the v2.0 snapshot gate fired.\n%s\n"+
		"If this change is a deliberate v2.0-additive change, regenerate the baseline:\n"+
		"  go test ./internal/apisnapshot/ -run TestAPISnapshot -update",
		diff.String())
}
